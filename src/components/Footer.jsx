...
import { MessageCircle, Navigation, MapPin, Phone, Mail, Heart } from 'lucide-react';
            ...
            <div className="max-w-[1300px] mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-4 pt-2">
                        <a href={createWhatsAppUrl(storeProfile)} target="_blank" rel="noopener noreferrer" aria-label={`Message ${storeProfile.shortName} on WhatsApp`} className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center hover:bg-orange-600 hover:text-white transition-all transform hover:scale-110">
                            <MessageCircle size={16} />
                        </a>
                        <a href={`tel:+91${storeProfile.primaryPhone}`} aria-label={`Call ${storeProfile.shortName}`} className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center hover:bg-orange-600 hover:text-white transition-all transform hover:scale-110">
                            <Phone size={16} />
                        </a>
                        <a href={createMapsUrl(storeProfile)} target="_blank" rel="noopener noreferrer" aria-label={`Get directions to ${storeProfile.shortName}`} className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center hover:bg-orange-600 hover:text-white transition-all transform hover:scale-110">
                            <Navigation size={16} />
                        </a>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <ul className="space-y-3 text-sm">
                        ...
                        <li><a href={createWhatsAppUrl(storeProfile)} target="_blank" rel="noopener noreferrer" className="hover:text-orange-500 transition-colors">Contact Us on WhatsApp</a></li>
                        <li><Link to="/track-order" className="hover:text-orange-500 transition-colors">Track Your Order</Link></li>
                        <li><span className="text-slate-500">In-store categories:</span></li>
                        <li className="text-slate-400 leading-relaxed">{storeProfile.categories.slice(0, 5).join(' • ')}</li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul className="space-y-4 text-sm">
                        <li className="flex items-start gap-3">
                            ...
                            <span>{storeProfile.addressLine1}<br />{storeProfile.addressLine2}<br />{storeProfile.state}, {storeProfile.country}</span>
                        </li>
                        <li className="flex items-center gap-3">
                            <Phone className="text-orange-500 shrink-0" size={18} />
                            <div className="flex flex-col">
                                <a href={`tel:+91${storeProfile.primaryPhone}`} className="hover:text-orange-500 transition-colors">+91 {storeProfile.primaryPhone}</a>
                                <a href={`tel:+91${storeProfile.secondaryPhone}`} className="hover:text-orange-500 transition-colors">+91 {storeProfile.secondaryPhone}</a>
                            </div>
                        </li>
                        <li className="flex items-center gap-3">
                            <Mail className="text-orange-500 shrink-0" size={18} />
                            <a href={`mailto:${storeProfile.email}`} className="hover:text-orange-500 transition-colors">{storeProfile.email}</a>
                        </li>
                    </ul>
                </div>
            </div>
